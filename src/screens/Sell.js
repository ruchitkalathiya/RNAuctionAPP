...
class Sell extends React.Component {
  ...
  showDatePicker = async (stateKey, options) => {
    try {
      ...
      if (action === DatePickerAndroid.dismissedAction) {
        ...
        var unixDate = date.getFullYear() + '-' + month_ + '-' + date.getDate();
        var day_ = date.getDate() + 1;
        var unixDate_ = date.getFullYear() + '-' + month_ + '-' + day_;
  ...
  showTimePickers = async (stateKey, options) => {
    try {
  ...
  validate() {
    if (
      ...
      this.state.auctionEndTime == null ||
      this.state.auctionEndDate == null ||
      ...
      /[^a-zA-Z0-9 ]/.test(this.state.productName) ||
      this.state.productName.length < 5
    ) {
      ...
      /[^0-9]/.test(this.state.price) ||
      parseInt(this.state.price) < 500
    ) {
      this.setState({
